...
public class FlashlightHandRig : MonoBehaviour
{
    [Header("Hand IK")]
    ...
    void LateUpdate()
    {
        if (active && gripPoint != null && leftHandTarget != null)
            leftHandTarget.SetPositionAndRotation(gripPoint.position, gripPoint.rotation);

        UpdateElbowHint();

        float targetRigWeight = active ? 1f : 0f;
    ...
    {
        if (elbowHint == null || cameraTransform == null) return;
